...
import ai.deepcode.android.data.remote.shouldIncludeTools
                ...
                for ((provider, modelId, apiKey, slotIndex) in resolvedFallback) {
                    ...
                    try {
                        ...
                        while (loopCount < maxLoops && !hasCompleted && !delivered) {
                            ...
                            val declaredTools = getAgentTools()
                            val toolsNeeded = loopCount < maxLoops && shouldIncludeTools(finalHistory, declaredTools)
                            val tools = if (toolsNeeded) declaredTools else emptyList()
