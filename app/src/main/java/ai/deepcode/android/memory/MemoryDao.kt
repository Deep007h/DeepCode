...
@Dao
interface MemoryDao {
    @Query("SELECT rowid, * FROM memory_chunks ORDER BY rowid DESC")
    fun getAllMemoryChunksFlow(): Flow<List<MemoryChunk>>

    @Query("SELECT rowid, * FROM memory_chunks")
    suspend fun getAllMemoryChunks(): List<MemoryChunk>

    @Query("SELECT rowid, * FROM memory_chunks WHERE content MATCH :query")
    ...
    @Query("SELECT rowid, * FROM memory_chunks ORDER BY rowid DESC LIMIT :limit")
    suspend fun getRecentMemories(limit: Int): List<MemoryChunk>

    @Query("SELECT rowid, * FROM memory_chunks WHERE source = :source ORDER BY rowid DESC")
    suspend fun getMemoriesBySource(source: String): List<MemoryChunk>
